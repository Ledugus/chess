...
class ChessGame(State):
    def __init__(self, game):
        ...
        self.switch_board_button = Button(
            image=p.image.load("images/switch.png"),
            pos=(self.BOARD_SIZE - 10, self.BOARD_SIZE + 10),
            text_input="",
            font=get_font(20),
            base_color="#d7fcd4",
        ...
        self.resign_button = Button(
            ...
            pos=(self.BOARD_SIZE - 200, self.BOARD_SIZE + 25),
            text_input="Resign",
            font=get_font(20),
            base_color="#d7fcd4",
            hovering_color="White",
    ...
    def update(self, actions):
        ...
        if actions["left"]:
            ...
            if not self.game_ended and self.resign_button.check_for_input(
                actions["mouse_pos"]
            ):
                self.gs.resign()
            if self.game_ended and self.new_game_button.check_for_input(
                actions["mouse_pos"]
            ):
                self.game.new_game()
            ...
            if row < 8 and col < 8:
                ...
                if len(self.player_clicks) == 2:
                    move = Move(
                        self.player_clicks[0], self.player_clicks[1], self.gs.board
                    )
        ...
        elif actions["molette_up"] and is_over(
            (self.BOARD_SIZE, 0, 200, self.BOARD_SIZE), actions["mouse_pos"]
        ):
            self.scroll_y = min(self.scroll_y + 10, 0)
        elif actions["molette_down"] and is_over(
            (self.BOARD_SIZE, 0, 200, self.BOARD_SIZE), actions["mouse_pos"]
        ):
    ...
    def update_possible_moves(self):
        if self.sq_selected == ():
            ...
            self.valid_moves = self.gs.get_valid_moves(
                self.gs.get_piece_moves(*self.sq_selected)
            )
            self.possible_moves = []
    ...
    def render(self, screen):
        ...
        self.switch_board_button.render(screen)
        if not self.game_ended:
            ...
            for c in range(8):
                ...
                if (r, c) in self.possible_moves:
                    ...
                    if self.gs.board[r][c] == "--":
                        ...
                        p.draw.circle(
                            screen, (0, 255, 0), pos, (self.SQ_SIZE - 10) / 2, 5
                        )
    ...
    def stats_display(self, screen):
        ...
        text = the_font.render(
            f"Matériel : ", True, p.Color("black"), p.Color(self.CHESS_BG_COLOR)
        )
        rect = text.get_rect()
        signe = ""
        if self.gs.count_material_delta() > 0:
            signe = "+ "
        text1 = the_font.render(
            signe + str(self.gs.count_material_delta()), True, p.Color("black")
        )
    ...
    def moves_display(self, screen):
        ...
        scrollable_moves = p.surface.Surface(
            (200, ((len(self.gs.move_log) // 2) + 1) * 30)
        )
        ...
        for i, move in enumerate(self.gs.move_log):
            ...
            scrollable_moves.blit(
                text_object, (100 * (i % 2) + 2, (30 * (i // 2)) + self.scroll_y)
            )
