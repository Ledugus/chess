...
class MainMenu(State):
    def __init__(self, game):
        ...
        self.BG = p.transform.scale(
            p.image.load("images/wp2883270.jpg"),
            (game.SCREEN_WIDTH, game.SCREEN_HEIGHT),
        )
        self.buttons = [
            Button(
                image=p.image.load("images/play_image.png"),
                pos=(game.SCREEN_WIDTH / 2, 250),
                text_input="PLAY",
                font=get_font(75),
                height=150,
            ),
    ...
    def update(self, actions):
        ...
        if actions["left_click"] and self.buttons[0].check_for_input(
            actions["mouse_pos"]
        ):
